    ...
    return (
      <div style={{ background: "#090909", minHeight: "100dvh" }}>
        <div className="max-w-xl mx-auto px-4 py-6" style={{ paddingTop: "max(24px, env(safe-area-inset-top,24px) + 16px)" }}>
          <button onClick={() => router.back()} className="flex items-center gap-1.5 mb-6 border-none bg-transparent cursor-pointer px-0" style={{ color: "#666" }}>
          ...
          <div className="flex flex-col items-center text-center py-20">
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4"
              ...
              <span className="material-symbols-outlined" style={{ fontSize: 32, color: "#7c3aed" }}>auto_awesome</span>
            </div>
            <h1 className="text-xl font-bold mb-2" style={{ color: "#f2f2f2" }}>AI Studio</h1>
    ...
    <div style={{ background: "#090909", minHeight: "100dvh" }}>
      <div className="max-w-xl mx-auto px-4 py-6" style={{ paddingTop: "max(24px, env(safe-area-inset-top,24px) + 16px)" }}>
        ...
        <div onClick={() => fileRef.current?.click()} className="rounded-2xl mb-4 flex flex-col items-center justify-center cursor-pointer overflow-hidden"
          ...
          {preview ? (
            ...
            <div className="flex flex-col items-center gap-2 py-10">
              ...
              <p className="text-xs" style={{ color: "#444" }}>{tab === "image" ? "JPG, PNG, WEBP" : "MP4, MOV up to 30s"}</p>
            </div>
          )}
        ...
        <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)}
          ...
          rows={2} className="w-full rounded-xl px-4 py-3 text-sm mb-4 resize-none outline-none"
        ...
        <button onClick={generate} disabled={loading || (!file && tab === "image")}
          ...
          {loading ? (
            <><div className="spinner" style={{ width: 16, height: 16 }} />Generating... this takes ~30 seconds</>
          ) : (
            ...
            <div className="px-4 py-3 flex items-center justify-between" style={{ background: "rgba(124,58,237,0.08)" }}>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => router.push(`/?aiResult=${encodeURIComponent(result)}`)}
                  ...
                  <span className="material-symbols-outlined" style={{ fontSize: 14 }}>add_circle</span>Post
                </button>
              </div>
            </div>
            {tab === "image"
