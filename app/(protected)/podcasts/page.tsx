    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
          ...
          <div className="w-full sm:max-w-sm rounded-t-3xl sm:rounded-2xl p-6"
            ...
            <div className="flex items-center gap-3 mb-5">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center"
                ...
                <span className="material-symbols-outlined" style={{ fontSize: 20, color: "#ef4444", fontVariationSettings: "'FILL' 1" }}>podcasts</span>
              </div>
              <div>
                <p className="text-sm font-bold" style={{ color: "#f2f2f2" }}>Go Live Podcast</p>
                <p className="text-xs" style={{ color: "#555" }}>Start a live audio session</p>
              </div>
            </div>
            <input type="text" placeholder="Session title (optional)"
            ...
            <button onClick={startLive} disabled={goingLive}
              ...
              style={{ background: goingLive ? "rgba(239,68,68,0.4)" : "#ef4444", color: "#fff" }}>
              {goingLive
                ? <><div className="spinner" style={{ width: 16, height: 16 }} /> Starting…</>
                : <><span className="material-symbols-outlined" style={{ fontSize: 16 }}>sensors</span> Go Live</>}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center"
          ...
          <div className="w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl p-6"
            ...
            <input type="text" placeholder="Episode title" value={uploadTitle}
              ...
              style={{ background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.1)", color: "#f2f2f2" }} />
            <textarea placeholder="Description (optional)" value={uploadDesc}
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "#555" }}>Audio episodes & live sessions</p>
        </div>
        {isOwner && (
      ...
      {liveShows.length > 0 && (
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-2 h-2 rounded-full" style={{ background: "#ef4444", boxShadow: "0 0 6px rgba(239,68,68,0.6)", animation: "pulse 1.5s infinite" }} />
            <span className="text-sm font-bold" style={{ color: "#f2f2f2" }}>Live Now</span>
          </div>
            ...
            {liveShows.map((show) => {
              ...
              return (
                <Link key={show.id} href={`/podcasts/live/${show.id}`}
                  ...
                  style={{ background: "#131313", border: `1px solid ${isMe ? "rgba(239,68,68,0.3)" : "rgba(239,68,68,0.15)"}` }}>
                  <div className="relative flex-shrink-0">
                    {show.hostPhoto ? (
                      <img src={show.hostPhoto} alt="" className="rounded-full object-cover" style={{ width: 48, height: 48 }} />
                    ) : (
                      <div className="rounded-full flex items-center justify-center font-bold"
                        style={{ width: 48, height: 48, background: "#222", color: "#aaa", fontSize: 18 }}>{init}</div>
                    )}
                    <div className="absolute -bottom-1 -right-1 px-1.5 py-0.5 rounded-full flex items-center gap-1"
                      ...
                      <span className="text-white font-bold" style={{ fontSize: 8 }}>{isMe ? "YOU" : "LIVE"}</span>
                    </div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold" style={{ color: "#f2f2f2" }}>{isMe ? "You" : show.hostName}</p>
                    {show.title && <p className="text-xs mt-0.5 truncate" style={{ color: "#666" }}>{show.title}</p>}
                    {show.listenerCount !== undefined && (
                      <div className="flex items-center gap-1 mt-1">
                        <span className="material-symbols-outlined" style={{ fontSize: 11, color: "#555" }}>headphones</span>
                        <span className="text-xs" style={{ color: "#555" }}>{show.listenerCount} listening</span>
                      </div>
                    )}
                  </div>
                  <span className="material-symbols-outlined flex-shrink-0" style={{ fontSize: 22, color: "#ef4444", fontVariationSettings: "'FILL' 1" }}>sensors</span>
                </Link>
              );
            })}
          </div>
      ...
      {/* Episodes */}
      <div>
        <p className="text-sm font-bold mb-3" style={{ color: "#f2f2f2" }}>Episodes</p>
        {episodesLoading ? (
            ...
            {episodes.map((ep) => (
              <Link key={ep.id} href={`/podcasts/${ep.id}`}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {ep.description && <p className="text-xs mt-0.5 truncate" style={{ color: "#666" }}>{ep.description}</p>}
                  <div className="flex items-center gap-2 mt-1.5">
                    {ep.duration && <span className="text-xs" style={{ color: "#555" }}>{formatDuration(ep.duration)}</span>}
                    {ep.duration && ep.publishedAt && <span style={{ color: "#333", fontSize: 10 }}>·</span>}
                    {ep.publishedAt && <span className="text-xs" style={{ color: "#555" }}>{timeAgo(ep.publishedAt.seconds)}</span>}
                  </div>
                </div>
